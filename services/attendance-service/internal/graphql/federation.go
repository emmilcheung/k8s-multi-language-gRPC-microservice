...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "AdmissionPass":
		resolverName, err := entityResolverNameForAdmissionPass(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "AdmissionPass": %w`, err)
		}
		switch resolverName {
		...
		case "findAdmissionPassByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findAdmissionPassByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindAdmissionPassByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "AdmissionPass": %w`, err)
			}
...
func entityResolverNameForAdmissionPass(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for AdmissionPass", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for AdmissionPass", ErrTypeNotFound))
			break
		}
		return "findAdmissionPassByID", nil
	}
	return "", fmt.Errorf("%w for AdmissionPass due to %v", ErrTypeNotFound,

...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "SeatingPlan":
		resolverName, err := entityResolverNameForSeatingPlan(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "SeatingPlan": %w`, err)
		}
		switch resolverName {
		...
		case "findSeatingPlanByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findSeatingPlanByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindSeatingPlanByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "SeatingPlan": %w`, err)
			}
...
func entityResolverNameForSeatingPlan(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for SeatingPlan", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for SeatingPlan", ErrTypeNotFound))
			break
		}
		return "findSeatingPlanByID", nil
	}
	return "", fmt.Errorf("%w for SeatingPlan due to %v", ErrTypeNotFound,
